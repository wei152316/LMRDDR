...
function [Dc,rc,dD,drc,d2psi] = NGFdotnew(Tc,Rc,omega,m,varargin)
...
edge1         = 100;                       % the edge parameter
edge2         = 100;                       % the edge parameter
...
lengthGT = sqrt(sum(gradT.^2,2) + edge1^2); % compute regularized length of dT[y] 
lengthGR = sqrt(sum(gradR.^2,2) + edge2^2); % compute regularized length of dR
      
r1  = sum(gradR.*gradT,2)+ edge1*edge2;                 % nominator
r2  = 1./(lengthGT.*lengthGR);             %  ... and denominator
rc  = r1 .* r2;                            % combine things and finalize
% Dc  = prod(omega(2:2:end)-omega(1:2:end)) - hd * (rc'*rc);
Dc  = length(gradT) - (rc'*rc);
