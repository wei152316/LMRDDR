...
%edited by cnx 因为这里图像补零了 所以要更新标记点
try
    main.LMinit = main.LM;%储存原始的点
    siz = size(main.LM);
    d = 2^(main.subdivide-1)*main.siz - dimen;
    tmp = repmat([d(2),d(1)],[siz(1),siz(2)/2]);
    main.LM = main.LM + tmp; %补零后的点
catch
end
...
for level=1:main.subdivide
    ...
    try
        tmp = (size(im)./main.siz);
        main.cof = tmp; %记录每次冲采样的系数
        siz = size(main.LM);
        main.LM = 1+(main.LMinit-1)./repmat(tmp(end:-1:1),[siz(1),siz(2)/2]); %等比例调整LM大小
    catch
    end
