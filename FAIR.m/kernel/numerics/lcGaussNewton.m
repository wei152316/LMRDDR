...
% -- end initialization   ------------------------------------------------
if dispimage == 1
    tmp = reshape(para.Tc,p(2),p(1));
    cnx = figure,
%     subplot(1,2,1); 
    imshow(tmp);title('Tc');hold on;
    tmp = reshape(para.Rc,p(2),p(1));
%     subplot(1,2,2);
    imshow(tmp);title('Rc');hold on;
end
%-- start the iteration --------------------------------------------------
while 1, 
  ...
  if dispimage == 1
      pause(0.1);
      tmp = reshape(para.Tc,p(2),p(1));
      subplot(1,2,1); imshow(tmp,[]);title('Tc');
      tmp = reshape(para.Rc,p(2),p(1));
      subplot(1,2,2); imshow(tmp,[]);title('Rc');
  end
%   Plots(iter,para);
% pause
