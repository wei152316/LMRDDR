...
function [Jc,para,dJ,H] = PIRobjFctn(T,Rc,omega,m,beta,M,wRef,xc,wc,p)
...
[Tc,dT] = imgModel(T,omega,yc,'doDerivative',1);
% Tmp = (T - mean(Tc))*sqrt((var(Rc)/var(Tc)))+mean(Rc);
% [Tc,dT] = imgModel(Tmp,omega,yc,'doDerivative',doDerivative);

% [Tc,dT] = imgModel(T,omega,yc,'doDerivative',1);
% distance('reset','distance','NGF','edge',edge);
[Dc,rc,dD,dr,d2psi] = distance(Tc,Rc,omega,m,'doDerivative',doDerivative,'m',p);
...
%显示局部区域 Tc和Rc paper用
 ...
 X=[x x(1)];       %首尾相接
 Y=[y y(1)];
% figure,imshow(reshape(Tc,[11,11]));hold on;
% plot(X,Y,'r','LineWidth',1.5);
% plot(6,6,'b+','LineWidth',1.5);
% figure,imshow(reshape(Rc,[11,11]));hold on;
% plot(X,Y,'r','LineWidth',1.5);
% plot(6,6,'b+','LineWidth',1.5);

% w = [cos(wc(1)),-sin(wc(1)),wc(2),sin(wc(1)),cos(wc(1)),wc(3)];
% q = kron(eye(2),[10.9121,57.9347,1]);
% new = q*w';
% add regularization
